...
# 探索训练数据量（需要调整的参数）
train_rate = 1.0  # train data 中使用的比例, 可以尝试 0.25, 0.5, 0.75, 1 等
...
best_model_dir = './results/lstm-util100/'  # 保存模型时使用
...
class Lab3_RNN(object):
    ...
    def train(self):
        ...
        logger.info(f">>> model type: {self.model_type}, train rate: {train_rate}")
        patience = 0
        best_val_acc = 0.
        best_train_acc = 0.
        for epoch in range(self.epochs):
            ...
            logger.info(f"epoch: {epoch}, train loss: {train_loss:.6f}, train acc: {train_acc:.6f}, "
                        f"val loss: {val_loss:.6f}, val acc: {val_acc:.6f},  time: {datetime.now() - t1}")
...
# RNN 训练
model_rnn = Lab3_RNN(lr=1e-3, epochs=10, device="cuda:3", hidden_size=100, sort_length=True,

...
# 探索训练数据量（需要调整的参数）
train_rate = 0.75  # train data 中使用的比例, 可以尝试 0.25, 0.5, 0.75, 1 等
...
best_model_dir = './results/rnn-util75/'  # 保存模型时使用
...
# RNN 训练
model_rnn = Lab3_RNN(lr=1e-3, epochs=10, device="cuda:2", hidden_size=100, sort_length=True,
                     patience=3, fix_embedding=True, num_layers=3, model_type='RNN')
model_rnn.train()
